...
export class MyDay implements IDay {
  ...
  public createNewDay(millisecondCount: number) {
    const weekString = moment(millisecondCount).isoWeek();
    const dayName = moment(millisecondCount).format(DateManager.DATE_FORMAT_V4);
    const yearName = dayName.split("/")[0];
    const isToday = dayName === DateManager.TODAY_TEXT_V2 || dayName === DateManager.TODAY_TEXT_V4;
...
export class DateManager {
  ...
  public static readonly YESTERDAY_OF_YEAR = moment(this.TODAY - this.DAY_MILLISECOND).dayOfYear();
  ...
  public static readonly WEEK_NUMBER = parseInt(moment().clone().startOf("month").add(1, "months").format("w"));

  public static readonly CURRENT_WEEK_NUMBER = moment().format("WW");

  public static readonly TODAY_TEXT = moment(this.TODAY).format(this.DATE_FORMAT);

  public static readonly TODAY_TEXT_V2 = moment(this.TODAY).format(this.DATE_FORMAT_V2);

  public static readonly TODAY_TEXT_V4 = moment(this.TODAY).format(this.DATE_FORMAT_V4);

  public static readonly FORMAT_DATE_BY_DOT = "DD.MM.YYYY";

  public static readonly THIS_MONTH_STRING = moment(this.TODAY).format(this.MONTH_NAME);

  public static readonly HOUR_LIST: string[] = (() => {
    const hourList: string[] = [];
    let startingPoint = new Date().setHours(0, 0, 0, 0);
    while (startingPoint < this.DAY_MILLISECOND + new Date().setHours(0, 0, 0, 0)) {
  ...
  public static readonly shortDayName = this.listDayBackWard.map((text: string) =>
    text.substring(0, 3).toLocaleLowerCase(),
  );
  ...
  public static readonly LAST_DAY_IN_MONTH = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getTime();

  public static readonly LAST_DAY_OF_THIS_MONTH_TEXT = moment(this.LAST_DAY_IN_MONTH).format(this.DATE_FORMAT);

  public static readonly LAST_DAY_OF_LAST_MONTH_TEXT = moment(this.LAST_DAY_IN_LAST_MONTH).format(this.DATE_FORMAT);
  ...
  private createDayList() {
    ...
    for (let i = 0; i < totalYear * 365 + leapYear; i++) {
      const dayMillisecondCount = i * DateManager.DAY_MILLISECOND + firstMonthOfArr;
      const myDay = new MyDay().createNewDay(dayMillisecondCount);
      this.dayList.push(myDay);
    }
    this.monthList = HelperManager.groupByV2(this.dayList, "monthName") as TMonth[];
    const monthListNameArr: any = Object.keys(this.monthList);
    const output: TMonth[] = [];

    const handleFullWeek = (monthName: keyof TMonth) => {
      let myMonth: TMonth = {};
      myMonth = { ...HelperManager.groupByV2((this.monthList as any)[monthName], "weekString") };
      ...
      for (const weekProperty of weekNumberStringArr) {
        ...
        if (dayListLength < 7) {
          const backWardDays = DateManager.listDayBackWard.indexOf(firstDay.format(DateManager.DATE_FORMAT_V3));
          for (let f = 1; f < backWardDays + 1; f++) {
            const dayMillisecondCount = firstDayMillionSeconds - f * DateManager.DAY_MILLISECOND;
          ...
          const forwardDays = DateManager.listDayForward.indexOf(
            moment(dayList[dayListLength - 1].millisecondCount).format(DateManager.DATE_FORMAT_V3),
          );

          for (let f = 1; f < forwardDays + 1; f++) {
            const dayMillisecondCount = dayList[dayListLength - 1].millisecondCount + f * DateManager.DAY_MILLISECOND;
        ...
        if (dayList.length === 6) {
          dayList.push(
            new MyDay().createNewDay(dayList[dayList.length - 1].millisecondCount + DateManager.DAY_MILLISECOND),
          );
        }

        const newSortDayList = dayList.sort((a, b) => a.millisecondCount - b.millisecondCount) as any;
