...
import styles from "./Styles.MultiSelectComponent";
import type { MultiSelectProps } from "./Model.MultiSelectComponent";
...
const MultiSelectComponent = React.forwardRef<any, MultiSelectProps>((props, currentRef) => {
  ...
  const [, setKey] = React.useState<number>(Math.random());
  ...
  const onSelect = React.useCallback(
    (item: any) => {
      const index = currentValue.findIndex((e) => e === lodash.get(item, valueField));
      if (index > -1) {
        currentValue.splice(index, 1);
      } else {
        if (maxSelect) {
          if (currentValue.length < maxSelect) {
            currentValue.push(lodash.get(item, valueField));
          }
        } else {
          currentValue.push(lodash.get(item, valueField));
        }
    ...
    return (
      <TouchableWithoutFeedback testID={testID} onPress={showOrClose}>
        <View style={styles.dropdown}>
          {renderLeftIcon?.()}
          <Text style={[styles.textItem, placeholderStyle, font()]}>{placeholder}</Text>
  ...
  const _renderItemSelected = (inside: boolean) => {
    const list = data.filter((e: any) => {
      const check = value?.indexOf(lodash.get(e, valueField));
      if (check !== -1) {
        return e;
      }
    });
      ...
      <View style={[styles.rowSelectedItem, inside && styles.flex1]}>
        {list.map((e) => {
          if (renderSelectedItem) {
            return (
              <TouchableOpacity
                testID={lodash.get(e, itemTestIDField || labelField)}
                key={HelperManager.idGenerator()}
                onPress={() => unSelect(e)}
              >
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => unSelect(e)}
              >
                <Text style={[styles.selectedTextLeftItem, selectedTextStyle, font()]}>
                  {lodash.get(e, labelField)}
                </Text>
